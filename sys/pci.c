...
uint64_t get_ahci_bar_address(uint8_t bus, uint8_t slot) {
   ...
   return (uint64_t)((abar5_2 << 16 | abar5_1 ) & (uint32_t)(0xFFFFF000));
	...
	while (i<32)
	{
		if (pi & 1)
		{
			...
			if (dt == AHCI_DEV_SATA)
			{
				...
				if(i == 1) {
				//	kprintf("call fun %x\n",((hba_mem_t *)(uint64_t)abar)->ghc);
					//kprintf("call fun %x\n",abar->ghc);
					kprintf("call fun\n");
					uint16_t buf1=7;
					uint16_t buf;
					read(&abar->ports[i], 0, 0, 1, &buf1,1);
					read(&abar->ports[i], 0, 0, 1, &buf,0);
					kprintf("value read : %x\n",buf);
				//	abar->ghc = 0;
				//	abar->ghc &= ~(1U << 31);
				//	kprintf("call fun %x\n",abar->ghc);
				//	((hba_mem_t *)(uint64_t)abar)->ghc = 1;
				//	port_rebase(&abar->ports[i],i,(uint64_t)abar);
				//	abar->ghc |= (1U << 31);
					//abar->ghc = 1;
...
void port_rebase(hba_port_t *port, int portno,uint64_t AHCI_BASE)
{
...
void stop_cmd(hba_port_t *port)
{
	...
	kprintf("stop cmd complete.\n");
}
uint8_t read(hba_port_t *port, DWORD startl, DWORD starth, DWORD count, WORD *buf,uint16_t readwrite)
{
	...
	cmdheader->w = readwrite;		// Read from device
