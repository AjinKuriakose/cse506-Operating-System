...
#	    define TMP_STDIN_NAME "/tmp/tcl.in.XXXXXX"
	    ...
	    my_strcpy(inName, TMP_STDIN_NAME);
	    mktemp(inName);
...
#	define TMP_STDERR_NAME "/tmp/tcl.err.XXXXXX"
	...
	my_strcpy(errName, TMP_STDERR_NAME);
	//mktemp(errName);
#if 0
	...
	*errFilePtr = open(errName, O_RDONLY, 0);
	...
	if (pid == 0) {
		...
		write(errorId < 0 ? 2 : errorId, err, my_strlen(err));
		_exit(1);
	    }
				 ...
				 * to structure about open file. */
{
	...
	if ((end == string+4) || (*end != 0)) {
	    ...
	    && (string[2] == 'd')) {
	if (my_strcmp(string+3, "in") == 0) {
	    fd = 0;
	} else if (my_strcmp(string+3, "out") == 0) {
	    fd = 1;
	} else if (my_strcmp(string+3, "err") == 0) {
	    fd = 2;
	} else {
	    goto badId;
	}
