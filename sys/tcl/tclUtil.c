				 ...
				 * of a Tcl_FreeProc such as free. */
{
	...
	if (length > TCL_RESULT_SIZE) {
	    ...
	    iPtr->freeProc = (Tcl_FreeProc *) Tcl_Free;
	} else {
	    iPtr->result = iPtr->resultSpace;
	    iPtr->freeProc = 0;
	}
	my_strcpy(iPtr->result, string);
    } else {
	iPtr->result = string;
    }
    ...
    if (oldFreeProc != 0) {
	if (oldFreeProc == (Tcl_FreeProc *) Tcl_Free) {
	    ckfree(oldResult);
	} else {
    ...
    //va_dcl
{
	...
	my_strcpy(iPtr->appendResult + iPtr->appendUsed, string);
	iPtr->appendUsed += my_strlen(string);
    }
    va_end(argList);
}
#endif
				 ...
				 * of new information may be added. */
{
	...
	iPtr->appendUsed = my_strlen(iPtr->result);
    }
	...
	my_strcpy(iPtr->appendResult, iPtr->result);
    }
    Tcl_FreeResult(iPtr);
					 ...
					 * compiled regular expression. */
{
	...
	if ((length == iPtr->patLengths[i])
		&& (my_strcmp(string, iPtr->patterns[i]) == 0)) {
	    /*
    ...
    my_strcpy(iPtr->patterns[0], string);
    iPtr->patLengths[0] = length;
    iPtr->regexps[0] = result;
    return result;
#endif
