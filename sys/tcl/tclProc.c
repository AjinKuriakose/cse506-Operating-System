    ...
    char **argv;			/* Argument strings. */
{
    ...
    procPtr->iPtr = iPtr;
    procPtr->command = (char *) ckalloc((unsigned) my_strlen(argv[3]) + 1);
    my_strcpy(procPtr->command, argv[3]);
    procPtr->argPtr = NULL;
	...
	nameLength = my_strlen(fieldValues[0]) + 1;
	if (fieldCount == 2) {
	    valueLength = my_strlen(fieldValues[1]) + 1;
	} else {
	    valueLength = 0;
	}
	...
	lastArgPtr = argPtr;
	argPtr->nextPtr = NULL;
	my_strcpy(argPtr->name, fieldValues[0]);
	if (fieldCount == 2) {
	    argPtr->defValue = argPtr->name + nameLength;
	    my_strcpy(argPtr->defValue, fieldValues[1]);
	} else {
