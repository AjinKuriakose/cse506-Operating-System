    ...
    char **argv;			/* Argument strings. */
{
    ...
    if ((c == 'd') && (my_strncmp(argv[1], "dirname", length) == 0)) {
	if (argc != 3) {
	    argv[1] = "dirname";
	    not3Args:
	    Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0],
	...
	return TCL_OK;
    } else if ((c == 'r') && (my_strncmp(argv[1], "rootname", length) == 0)
	    && (length >= 2)) {
	char *lastSlash;
	...
	return TCL_OK;
    } else if ((c == 'e') && (my_strncmp(argv[1], "extension", length) == 0)
	    && (length >= 3)) {
	char *lastSlash;
	...
	return TCL_OK;
    } else if ((c == 't') && (my_strncmp(argv[1], "tail", length) == 0)
	    && (length >= 2)) {
	if (argc != 3) {
	    argv[1] = "tail";
	    goto not3Args;
    ...
    if ((c == 'r') && (my_strncmp(argv[1], "readable", length) == 0)
	    && (length >= 5)) {
	if (argc != 3) {
	    argv[1] = "readable";
	    goto not3Args;
	...
	return TCL_OK;
    } else if ((c == 'w') && (my_strncmp(argv[1], "writable", length) == 0)) {
	if (argc != 3) {
	    argv[1] = "writable";
	    goto not3Args;
	}
	mode = W_OK;
	goto checkAccess;
    } else if ((c == 'e') && (my_strncmp(argv[1], "executable", length) == 0)
	    && (length >= 3)) {
	if (argc != 3) {
	    argv[1] = "executable";
	    goto not3Args;
	...
	mode = X_OK;
	goto checkAccess;
    } else if ((c == 'e') && (my_strncmp(argv[1], "exists", length) == 0)
	    && (length >= 3)) {
	if (argc != 3) {
	    argv[1] = "exists";
	    goto not3Args;
    ...
    if ((c == 'a') && (my_strncmp(argv[1], "atime", length) == 0)) {
	if (argc != 3) {
	    argv[1] = "atime";
	    goto not3Args;
	}
	...
	statOp = 2;
    } else if ((c == 'i') && (my_strncmp(argv[1], "isfile", length) == 0)
	    && (length >= 3)) {
	if (argc != 3) {
	    argv[1] = "isfile";
	    goto not3Args;
    ...
    } else if ((c == 'm') && (my_strncmp(argv[1], "mtime", length) == 0)) {
	if (argc != 3) {
	    argv[1] = "mtime";
	    goto not3Args;
	}
     ...
     */
    } else if ((c == 'r') && (my_strncmp(argv[1], "readlink", length) == 0)
	    && (length >= 5)) {
	...
	return TCL_OK;
#endif
    } else if ((c == 's') && (my_strncmp(argv[1], "size", length) == 0)
	    && (length >= 2)) {
	if (argc != 3) {
	    argv[1] = "size";
	    goto not3Args;
	...
	return StoreStatData(interp, argv[3], &statBuf);
    } else if ((c == 't') && (my_strncmp(argv[1], "type", length) == 0)
	    && (length >= 2)) {
	if (argc != 3) {
	    argv[1] = "type";
	    goto not3Args;
...
#endif
#endif
	currentDir = (char *) ckalloc((unsigned) (my_strlen(buffer) + 1));
	my_strcpy(currentDir, buffer);
    }
...
//    FILE *f;
    ...
    if (i == (argc-3)) {
	if (my_strncmp(argv[i+2], "nonewline", my_strlen(argv[i+2])) != 0) {
...
#define READ_BUF_SIZE 4096
    ...
    i = 1;
    newline = 1;
    if ((argc == 3) && (my_strcmp(argv[1], "-nonewline") == 0)) {
	newline = 0;
	i++;
    }
    ...
    char **argv;			/* Argument strings. */
{
	...
	length = my_strlen(argv[3]);
	c = argv[3][0];
	if ((c == 's') && (my_strncmp(argv[3], "start", length) == 0)) {
	    mode = SEEK_SET;
	} else if ((c == 'c') && (my_strncmp(argv[3], "current", length) == 0)) {
	    mode = SEEK_CUR;
	} else if ((c == 'e') && (my_strncmp(argv[3], "end", length) == 0)) {
	    mode = SEEK_END;
	} else {
...
#	    define BUFFER_SIZE 1000
    ...
    length = my_strlen(interp->result);
