...
#define STATIC_SIZE 200
	...
	for (p = openBrace; *p != '}'; ) {
	    ...
	    my_strcpy(newRem+l1+l2, closeBrace+1);
	...
	while (1) {
		...
		int nameLength = my_strlen(entryPtr->d_name);
		if (*p == 0) {
...
#define STATIC_BUF_SIZE 50
    ...
    length = my_strlen(dir) + my_strlen(p);
    if (length >= curSize) {
	if (curBuf != staticBuf) {
	    ckfree(curBuf);
	}
    ...
    char **argv;			/* Argument strings. */
{
    ...
    noComplain = 0;
    if ((argv[1][0] == '-') && (my_strcmp(argv[1], "-nocomplain") == 0)) {
	if (argc < 3) {
	    goto notEnoughArgs;
	}
	noComplain = 1;
